...
**3. Register Page**
A site user to the site who would like to sign up for an account, they simply need to provide their `email`, a `username` and a `password` with `password confirmation`. Once the user successfully registers they can follow the "My Profile" link to fill in their contact details.
