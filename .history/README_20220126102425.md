...
**1. Home Page** 
The main site page contains a banner images and some informative text as well as links and buttons helping the user to navigate the site. The User can choose to shop and search the site in an anonymous mode or decide to register.Navbar adapts depending on whether the user is logged in or not, offering different options. The is a search bar (in both cases) to allow a logged in user and someone who isn't logged in to browse the products (but not purchase unless they are an authenticated user)
