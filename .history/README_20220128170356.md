...
- Page Links Summary:
    ...
    •   My Profile - Upon clicking these buttons, the user can easily navigate to other pages on the site and thus the user experience is improved. An obvious and simple example is a user on the home page, wanting to see their profile can either navigate to the profile via the Navbar option, the aforementioned button or even the url, if they so choose.
